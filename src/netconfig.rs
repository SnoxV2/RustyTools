...
/// Collecte la configuration réseau du poste (interfaces, DNS, passerelles, routes).
pub fn gather() -> NetReport {
    ...
    // Interfaces actives et par défaut en tête de liste.
...
/// Domaine DNS et serveurs DNS complémentaires, selon l'OS.
...
fn routes_text() -> String {
    ...
    #[cfg(all(unix, not(target_os = "macos")))]
    {
        ...
        if v4.starts_with("Impossible d'exécuter") {
...
/// Sorties brutes des outils système, pour vérification ou copier-coller.
...
/// Rapport texte complet, pour l'export fichier.
pub fn report_text(report: &NetReport) -> String {
    let mut s = String::new();
    s.push_str(&format!(
        "RAPPORT DE CONFIGURATION RÉSEAU — généré le {}\n\n",
        report.generated_at
    ));
    s.push_str(&format!("Nom d'hôte : {}\n", report.hostname));
    s.push_str(&format!(
        "Domaine    : {}\n",
        report.domain.as_deref().unwrap_or("(aucun)")
    ));
    s.push_str(&format!(
        "DNS        : {}\n\n",
    ...
    for itf in &report.interfaces {
        s.push_str(&format!(
            ...
            itf.friendly_name
                ...
                .map(|f| format!(" ({f})"))
                .unwrap_or_default(),
            if itf.is_default { "  [interface par défaut]" } else { "" }
        ));
        s.push_str(&format!("  État      : {}\n", if itf.is_up { "UP" } else { "DOWN" }));
        ...
        if let Some(gw) = &itf.gateway {
            s.push_str(&format!("  Passerelle: {gw}\n"));
        }
        if !itf.dns.is_empty() {
...
/// Exporte le rapport dans le répertoire de logs et retourne le chemin du fichier.
